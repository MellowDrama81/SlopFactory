...
1min.AI's adapter, the full asynchronous-status vocabulary (Monitoring Paused, Submission Outcome Unknown, idempotency keys and reconciliation), named per-modality source-input slots for audio/video, the remaining Provider Safety Responses machinery, the full prompt-improvement opt-in/disclosure model, and the remaining connection-state distinctions (**Secure Storage Unavailable**, **Authentication Failed**) remain under development. The outstanding product requirements are tracked in [plan.md](plan.md), [milestone2.md](milestone2.md) and [milestone3.md](milestone3.md).
