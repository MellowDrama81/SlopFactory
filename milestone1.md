...
- [ ] Persist immediate read-only provenance for Duplicate and Edit as Copy; verify it points to the direct source and does not create transitive links.
- [ ] Add a read-only provenance-chain view that stops safely at missing/non-restorable endpoints; verify rename/move does not break ID-based traversal.
