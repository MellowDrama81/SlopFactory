...
The current 66 tests cover platform-version policy, initialization, manifest/database creation, copied-library adoption, local-storage path rejection, hard-link rejection, and open-library identity revalidation, exclusive locking, invalid library entries, safe managed import, hashing, progress, cancellation cleanup, single and bulk duplicate handling with direct provenance, folder/file organization, reviewed multi-file operations, bulk metadata sensitivity changes, typed metadata ownership and filtering, redacted JSON validation, and privacy-minimised integrity-report serialization.
