...
- `file_content_provenance`: immutable original hash, byte size, and media type plus the latest differing-replacement timestamp;
...
After obtaining the exclusive lock, `ValidateStorageCapabilities` creates a unique staging probe, performs a write-through byte write, atomically renames it within staging, and removes it. Failure is a validation error and cleanup attempts remove both names. Creation invokes the probe before database/manifest creation, while opening invokes it before schema work.
...
An external candidate is copied and hashed into `.staging`, the existing managed path is moved to a unique rollback path, and the staged file is installed before the database transaction. A database failure restores the previous managed bytes. Accepting the already-present changed bytes performs no filesystem replacement. After a successful database commit, transient rollback content is deleted; SlopFactory does not expose it as a version or provide replacement undo.
...
Sensitive-value copy is available only while that entry is revealed and uses the platform clipboard directly. The completion message contains no value, and the application does not log clipboard content or attempt automatic clipboard clearing.
...
`IntegrityReportExporter.SerializeDefault` serializes the report as `slopfactory.integrity-report/v1`. Its fixed export shape contains only library ID, schema version, start/finish instants, complete/cancelled flags, and finding category/opaque record ID/expected and actual byte sizes/summary. The settings page previews this fixed disclosure set and writes a temporary JSON file only for the platform share/save request. No names, paths, digests, metadata, prompts, secrets, authorization data, or file bytes enter that file.
...
`ManagedFileSafety` supplements reparse-point checks with Windows `GetFileInformationByHandle` link-count inspection. A managed regular file with more than one link is unsafe because a subsequent write or delete could affect an external path. Revalidation marks it changed, verified viewing and managed replacement reject it, recycle restoration and permanent deletion stop, and integrity scanning returns `UnsafeManagedEntry`. Platforms that do not expose an equivalent reliable link count retain the existing no-follow and containment checks.
