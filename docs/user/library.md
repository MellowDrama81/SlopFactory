...
**Export** uses a system-selected destination, writes a temporary sibling, verifies the copied byte count and SHA-256 hash, and only then commits it. Existing destinations are never silently renamed or partly replaced. Bulk export maps safe names in a preflight and requires an explicit collision choice for every existing destination. Cancelling cleans the active temporary copy and reports remaining items separately. If an export is interrupted by a crash or a forced application exit, SlopFactory cleans up any leftover temporary file the next time it starts rather than leaving it behind.

For a file created by AI generation, **Export** can optionally write a companion `.slopfactory.json` sidecar alongside the exported file, describing its origin. Every disclosure is off by default and must be turned on explicitly: the sidecar always identifies the media type, size, and content hash, and separate checkboxes let you also include the prompt and system instructions, original/display filenames, internal library identifiers, usage and cost, and advanced generation settings. A **Include provider safety metadata** checkbox is present but currently always a documented no-op, since SlopFactory doesn't yet persist a provider safety classification to disclose. **Preview sidecar** shows the exact JSON a real export would write before you commit to it. The sidecar's commit is tracked independently of the media file's, so a sidecar failure never affects an already-successful media export.
...
Open **Diagnostics** to see a rolling, device-local log of recent generation and storage problems — each entry shows an operation type, timestamp, and a sanitized error message, with an HTTP status code when one applies. Entries older than 30 days are removed automatically, and the log is capped to a bounded number of entries beyond that. Prompts, source content, and provider responses are never written to this log; only local failures (storage errors, connectivity, validation) and the fact that a submission failed are recorded.

If SlopFactory didn't close normally last time (a crash or a forced exit), the next launch shows a dismissible notice linking to Diagnostics. **Export diagnostics** saves the current log as a JSON file for troubleshooting; **Clear diagnostics** deletes it. Neither action affects your library data.
