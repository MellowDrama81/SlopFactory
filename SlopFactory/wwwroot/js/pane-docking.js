(function () {
  ...
  const loadPageExportImage=source=>new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>reject(new Error('The panel artwork could not be loaded.'));image.src=source;});
  const drawPageExportPanel=async(context,panel,x,y,width,height,source)=>{if(source){try{const image=await loadPageExportImage(source);context.save();context.beginPath();context.rect(x,y,width,height);context.clip();context.drawImage(image,x,y,width,height);context.restore();}catch{/* Retain the readable panel fallback below. */}}else{context.fillStyle=panel?.classList?.contains('is-placeholder')||panel?.kind==='placeholder'?'#e8efeb':'#dff1e9';context.fillRect(x,y,width,height);}context.strokeStyle=panel?.classList?.contains('is-placeholder')||panel?.kind==='placeholder'?'#76968e':'#5d9689';context.lineWidth=2;context.strokeRect(x,y,width,height);};
  Object.assign(window.recursiveDock,{rasterizePage:async(element,mime,maxDimension)=>{const bounds=element.getBoundingClientRect(),factor=maxDimension?Math.min(1,maxDimension/Math.max(bounds.width,bounds.height)):1,width=Math.max(1,Math.round(bounds.width*factor)),height=Math.max(1,Math.round(bounds.height*factor)),canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=width;canvas.height=height;context.scale(factor,factor);context.fillStyle='#f8f5ed';context.fillRect(0,0,bounds.width,bounds.height);await Promise.all([...element.querySelectorAll('.page-editor-panel')].map(async panel=>{const rect=panel.getBoundingClientRect(),art=panel.querySelector('.page-editor-panel-art');await drawPageExportPanel(context,panel,rect.left-bounds.left,rect.top-bounds.top,rect.width,rect.height,art?.currentSrc||art?.src);const label=panel.querySelector('span')?.textContent;if(label){context.fillStyle='#24574b';context.font='bold 12px sans-serif';context.textAlign='center';context.fillText(label,rect.left-bounds.left+rect.width/2,rect.top-bounds.top+rect.height/2);}}));return {dataUrl:canvas.toDataURL(mime,.92),width,height};},rasterizeBook:async(book,previews,maxDimension)=>await Promise.all((book.pages??[]).map(async page=>{const pageWidth=(book.pageWidthCm??21)*(page.layout==='double'?2:1),pageHeight=book.pageHeightCm??29.7,factor=maxDimension/Math.max(pageWidth,pageHeight),width=Math.max(1,Math.round(pageWidth*factor)),height=Math.max(1,Math.round(pageHeight*factor)),canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=width;canvas.height=height;context.fillStyle='#f8f5ed';context.fillRect(0,0,width,height);await Promise.all((page.panels??[]).map(async item=>{const x=(item.xcm??0)/pageWidth*width,y=(item.ycm??0)/pageHeight*height,w=(item.widthCm??0)/pageWidth*width,h=(item.heightCm??0)/pageHeight*height;await drawPageExportPanel(context,item,x,y,w,h,previews?.[item.id]);if(!previews?.[item.id]){context.fillStyle='#24574b';context.font='bold 12px sans-serif';context.textAlign='center';context.fillText(item.panelName??'Panel',x+w/2,y+h/2);}}));return {dataUrl:canvas.toDataURL('image/jpeg',.92),width,height};}))});
  const exportNumber=value=>{const parsed=Number.parseFloat(value);return Number.isFinite(parsed)?parsed:0;};
  const exportPanelGeometry=(record,baseX,baseY,baseWidth,baseHeight,style)=>{const value=(name,cssName)=>record?exportNumber(record[name]):exportNumber(style?.getPropertyValue(cssName));const scale=Math.max(.01,value('scale','--panel-scale')||1);const left=Math.min(100,Math.max(0,value('cropLeft','--crop-left'))),top=Math.min(100,Math.max(0,value('cropTop','--crop-top'))),right=Math.min(100-left,Math.max(0,value('cropRight','--crop-right'))),bottom=Math.min(100-top,Math.max(0,value('cropBottom','--crop-bottom'))),width=baseWidth*scale,height=baseHeight*scale,x=baseX+(baseWidth-width)/2,y=baseY+(baseHeight-height)/2;return {x,y,width,height,clipX:x+width*left/100,clipY:y+height*top/100,clipWidth:width*(100-left-right)/100,clipHeight:height*(100-top-bottom)/100};};
  const drawCroppedPageExportPanel=async(context,panel,geometry,source,label)=>{context.save();context.beginPath();context.rect(geometry.clipX,geometry.clipY,geometry.clipWidth,geometry.clipHeight);context.clip();let rendered=false;if(source){try{const image=await loadPageExportImage(source);context.drawImage(image,geometry.x,geometry.y,geometry.width,geometry.height);rendered=true;}catch{/* Fall back to the panel surface. */}}if(!rendered){context.fillStyle=panel?.classList?.contains('is-placeholder')||panel?.kind==='placeholder'?'#e8efeb':'#dff1e9';context.fillRect(geometry.x,geometry.y,geometry.width,geometry.height);}context.strokeStyle=panel?.classList?.contains('is-placeholder')||panel?.kind==='placeholder'?'#76968e':'#5d9689';context.lineWidth=2;context.strokeRect(geometry.x,geometry.y,geometry.width,geometry.height);if(label){context.fillStyle='#24574b';context.font='bold 12px sans-serif';context.textAlign='center';context.fillText(label,geometry.x+geometry.width/2,geometry.y+geometry.height/2);}context.restore();};
  Object.assign(window.recursiveDock,{rasterizePage:async(element,mime,maxDimension)=>{const bounds=element.getBoundingClientRect(),factor=maxDimension?Math.min(1,maxDimension/Math.max(bounds.width,bounds.height)):1,width=Math.max(1,Math.round(bounds.width*factor)),height=Math.max(1,Math.round(bounds.height*factor)),canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=width;canvas.height=height;context.scale(factor,factor);context.fillStyle='#f8f5ed';context.fillRect(0,0,bounds.width,bounds.height);for(const panel of element.querySelectorAll('.page-editor-panel')){const rect=panel.getBoundingClientRect(),style=getComputedStyle(panel),scale=Math.max(.01,exportNumber(style.getPropertyValue('--panel-scale'))||1),baseWidth=panel.offsetWidth,baseHeight=panel.offsetHeight,baseX=rect.left-bounds.left-(baseWidth*scale-baseWidth)/2,baseY=rect.top-bounds.top-(baseHeight*scale-baseHeight)/2,geometry=exportPanelGeometry(null,baseX,baseY,baseWidth,baseHeight,style),art=panel.querySelector('.page-editor-panel-art');await drawCroppedPageExportPanel(context,panel,geometry,art?.currentSrc||art?.src,panel.querySelector('span')?.textContent);}return {dataUrl:canvas.toDataURL(mime,.92),width,height};},rasterizeBook:async(book,previews,maxDimension)=>await Promise.all((book.pages??[]).map(async page=>{const pageWidth=(book.pageWidthCm??21)*(page.layout==='double'?2:1),pageHeight=book.pageHeightCm??29.7,factor=maxDimension/Math.max(pageWidth,pageHeight),width=Math.max(1,Math.round(pageWidth*factor)),height=Math.max(1,Math.round(pageHeight*factor)),canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=width;canvas.height=height;context.fillStyle='#f8f5ed';context.fillRect(0,0,width,height);for(const item of page.panels??[]){const baseX=(item.xcm??0)/pageWidth*width,baseY=(item.ycm??0)/pageHeight*height,baseWidth=(item.widthCm??0)/pageWidth*width,baseHeight=(item.heightCm??0)/pageHeight*height,geometry=exportPanelGeometry(item,baseX,baseY,baseWidth,baseHeight);await drawCroppedPageExportPanel(context,item,geometry,previews?.[item.id],item.panelName??'Panel');}return {dataUrl:canvas.toDataURL('image/jpeg',.92),width,height};}))});
})();
