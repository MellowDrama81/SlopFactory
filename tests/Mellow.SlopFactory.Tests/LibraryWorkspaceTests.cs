...
public sealed class LibraryWorkspaceTests
{
    ...
    public async Task OpeningVersionOneLibraryUpgradesDatabaseAndManifestWithRollbackCleanup()
    {
        ...
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 1", StringComparison.Ordinal));

        await using var upgraded = await factory.OpenAsync(root);

        Assert.Equal(28, upgraded.Descriptor.SchemaVersion);
        ...
        Assert.Contains("\"schemaVersion\": 28", await File.ReadAllTextAsync(manifestPath));
    }
    ...
    public async Task OpeningVersionTwoLibraryAddsPermanentDeletionFailureStorage()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 2", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionThreeLibraryAddsOriginalFilenameFromDisplayName()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 3", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionFourLibraryAddsContentHealthState()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 4", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionFiveLibraryAddsImmutableContentProvenance()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 5", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionFourteenLibraryAddsModelCatalogueCache()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 14", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionFifteenLibraryAddsConnectionTimeoutOverride()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 15", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionSixteenLibraryAddsConnectionHeaders()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 16", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionSeventeenLibraryAddsGenericModalitySettings()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 17", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionEighteenLibraryAddsPromptImprovementHistory()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 18", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionNineteenLibraryAddsNeedsReviewColumns()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 19", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentyLibraryAddsTextResultFormat()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 20", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentyOneLibraryAddsGenerationDrafts()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 21", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentyTwoLibraryAddsCredentialRevisionLedger()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 22", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentyThreeLibraryAddsSavedSettingRevision()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 23", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentyFourLibraryFixesGenerationResultFileDeletionCascade()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 24", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentyFiveLibraryAddsGenerationRecordLifecycleAndTombstoneColumns()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 25", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentySixLibraryAddsGenerationSettingsColumns()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 26", StringComparison.Ordinal));
    ...
    public async Task OpeningVersionTwentySevenLibraryAddsSecondaryAndTertiarySourceSlots()
    {
        ...
        var manifestPath = Path.Combine(root, "slopfactory-library.json");
        var manifest = await File.ReadAllTextAsync(manifestPath);
        await File.WriteAllTextAsync(manifestPath, manifest.Replace("\"schemaVersion\": 28", "\"schemaVersion\": 27", StringComparison.Ordinal));
