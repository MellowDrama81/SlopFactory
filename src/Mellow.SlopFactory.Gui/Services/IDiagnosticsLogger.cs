...
/// free-text message — API keys, authorization headers, raw or improved
/// prompts, system instructions, prompt-improvement guidance, source/generated-file contents, signed
/// result URLs, provider moderation categories/descriptions, and sensitive user-metadata values must
/// never appear in diagnostics. A structured record with no arbitrary-text field makes most of
/// that impossible to violate by construction rather than relying on every call site remembering to
/// sanitize a string; <see cref="SanitizedError"/> is the one free-text field permitted
...
public interface IDiagnosticsLogger
{
    ...
    /// rolling cap — whichever removes an entry first, applied together on every
    ...
    /// <summary>True while verbose logging is active and not yet expired.</summary>
    bool VerboseEnabled { get; }

    /// <summary>When the current verbose period ends, or null if verbose logging is off. Persisted
    /// so the expiry survives an application restart.</summary>
    DateTimeOffset? VerboseExpiresAt { get; }

    /// <summary>Activates verbose logging for exactly one hour from now —
    ...
    /// graceful shutdown. Call <see cref="MarkSessionStarted"/> once at startup
    /// before reading this.
    /// </summary>
    ...
    /// normally (crash detection only fires when this was never called last time).</summary>
