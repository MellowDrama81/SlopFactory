...
/// <summary>
/// Together AI (`api.together.xyz/v1`) exposes an OpenAI-compatible `chat/completions` endpoint and a
/// plain `images/generations`-shaped endpoint (hosting FLUX and other open image models) — the
/// broadest single-provider modality match to this app's four-surface adapter shape among the
/// OpenAI-compatible candidates surveyed in providers.md, but only text-to-image generation is
/// confirmed there; no image-edit (reference-image) endpoint shape was documented, so
/// <see cref="Mellow.SlopFactory.Domain.LibraryRules.GetInputSlotCapabilities"/> does not offer a
/// reference-image slot for this provider's Image mode and <see cref="GenerateImageAsync"/> never
/// receives source images to translate. Audio/video are also hosted per providers.md, but their
/// request/response shapes were not confirmed there and are not implemented.
/// </summary>
internal sealed class TogetherAiProviderAdapter : IProviderAdapter
{
    private readonly HttpClient _httpClient;
    private readonly IConnectionRateLimitTracker? _rateLimitTracker;

    public TogetherAiProviderAdapter(HttpClient httpClient, IConnectionRateLimitTracker? rateLimitTracker = null)
    {
        _httpClient = httpClient;
        _rateLimitTracker = rateLimitTracker;
    }

    public ProviderType ProviderType => ProviderType.TogetherAi;
    ...
    public async Task<IReadOnlyList<byte[]>> GenerateImageAsync(Connection connection, Model model, string? apiKey, string prompt, int resultCount, IReadOnlyList<TextGenerationSourceImage>? sourceImages = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ProviderAdapterException("Together AI's image-edit request shape has not been confirmed; this model only supports plain text-to-image generation.");
        }
    ...
    public Task<IReadOnlyList<byte[]>> GenerateAudioAsync(Connection connection, Model model, string? apiKey, string prompt, int resultCount, string? voice = null, CancellationToken cancellationToken = default) =>
        throw new ProviderAdapterException("Audio generation is not implemented for Together AI: its TTS/STT endpoint shapes were not confirmed.");
    ...
    public Task<AsyncGenerationSubmission> SubmitVideoGenerationAsync(Connection connection, Model model, string? apiKey, string prompt, TextGenerationSourceImage? firstFrame = null, CancellationToken cancellationToken = default) =>
        throw new ProviderAdapterException("Video generation is not implemented for Together AI: its video-generation endpoint shape was not confirmed.");
