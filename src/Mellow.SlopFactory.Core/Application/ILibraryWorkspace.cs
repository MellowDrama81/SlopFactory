...
public interface ILibraryWorkspace : IAsyncDisposable
{
    ...
    Task<GenerationRecord> RecordMediaGenerationResultAsync(string modelId, string prompt, int resultCount, string destinationFolderId, IReadOnlyList<byte[]>? resultFiles, string? errorMessage, string? promptImprovementRecordId = null, CancellationToken cancellationToken = default);
