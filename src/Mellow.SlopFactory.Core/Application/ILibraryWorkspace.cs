...
public interface ILibraryWorkspace : IAsyncDisposable
{
    ...
    Task<Model> CreateModelAsync(string label, string connectionId, string providerModelId, GenerationMode mode, bool supportsSystemInstructions, TextResultFormat textFormat = TextResultFormat.Markdown, string? comfyWorkflowTemplate = null, CancellationToken cancellationToken = default);
    Task<Model> UpdateModelAsync(string modelId, string label, string providerModelId, GenerationMode mode, bool supportsSystemInstructions, TextResultFormat textFormat = TextResultFormat.Markdown, string? comfyWorkflowTemplate = null, CancellationToken cancellationToken = default);
    ...
    /// <summary>Creates a durable <see cref="GenerationStatus.Queued"/> generation record before any
    /// provider work happens, so the record itself is the snapshot restart recovery reads back.</summary>
    Task<GenerationRecord> CreateQueuedGenerationRecordAsync(string modelId, string prompt, int resultCount, string destinationFolderId, string? systemInstructions = null, IReadOnlyList<GenerationSourceSlot>? sourceSlots = null, GenerationSettings? settings = null, string? promptImprovementRecordId = null, CancellationToken cancellationToken = default);
    /// <summary>Advances a generation record's status and logs a timestamped transition; throws if
    /// the record is already terminal.</summary>
    Task<GenerationRecord> AdvanceGenerationStatusAsync(string generationRecordId, GenerationStatus status, GenerationHoldReason? holdReason = null, GenerationFailureReason? failureReason = null, int? position = null, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<GenerationStatusTransition>> GetGenerationStatusHistoryAsync(string generationRecordId, CancellationToken cancellationToken = default);
    /// <summary>Abandon Recovery: finalizes a <see cref="GenerationStatus.SubmissionOutcomeUnknown"/>
    /// or <see cref="GenerationStatus.Paused"/> record to <see cref="GenerationStatus.Failed"/> when
    /// the user gives up on it ever being resolved. Throws for any other status.</summary>
    Task<GenerationRecord> AbandonGenerationOutcomeAsync(string generationRecordId, CancellationToken cancellationToken = default);
    ...
    Task<SavedGenerationSetting> CreateSavedSettingAsync(string title, string? modelId, string prompt, int resultCount, string destinationFolderId, string? systemInstructions = null, GenerationSettings? settings = null, IReadOnlyList<GenerationSourceSlot>? sourceSlots = null, CancellationToken cancellationToken = default);
    ...
    Task<IReadOnlyList<AsyncRemoteJobRecord>> GetAsyncRemoteJobsForGenerationRecordAsync(string generationRecordId, CancellationToken cancellationToken = default);
    Task<AsyncRemoteJobRecord> LinkAsyncRemoteJobToGenerationResultAsync(string asyncJobId, string generationRecordId, int position, CancellationToken cancellationToken = default);
    ...
    Task<FileRecord> ImportMissingResultAsync(string generationRecordId, int position, byte[] bytes, CancellationToken cancellationToken = default);
}
